...
1. Minimum required version: Visual Studio 2026 version 18.0.
...
```powershell
cmake -B build -T v145 --preset vs2022-static  # It might take a while if the vcpkg binary cache is unpopulated or invalidated.
...
```powershell
cmake -B build -T v145 --preset vs2022 -DBUILD_GUI=OFF # It might take a while if the vcpkg binary cache is unpopulated or invalidated.
cmake --build build --config Release                   # Append "-j N" for N parallel jobs.
ctest --test-dir build --build-config Release          # Append "-j N" for N parallel tests.
```
...
```powershell
cmake -B build -T v145 --preset vs2022-static -DVCPKG_INSTALL_OPTIONS="--x-buildtrees-root=C:\vcpkg"
```
...
```powershell
cmake -B build -T v145 --preset vs2022-static -DVCPKG_INSTALLED_DIR="C:\path_without_spaces"
```
...
```
cmake -B build -T v145 --preset vs2022 -DVCPKG_MANIFEST_NO_DEFAULT_FEATURES=ON -DVCPKG_MANIFEST_FEATURES="wallet;tests" -DBUILD_GUI=OFF
```
