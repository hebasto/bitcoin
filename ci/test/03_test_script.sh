...
if [[ "${RUN_IWYU}" == true ]]; then
  ...
  FILES_WITH_ENFORCED_IWYU='/src/((bench|common|consensus|crypto|index|init|kernel|primitives|script|univalue/(lib|test)|util|zmq)/.*|node/(blockstorage|interfaces|miner|mining_args|utxo_snapshot)|rpc/mining|test/fuzz/(kitchen_sink|parse_univalue)|clientversion|core_io|signet|rest|init)\.cpp'
