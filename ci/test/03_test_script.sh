...
if [[ "${RUN_IWYU}" == true ]]; then
  ...
  FILES_WITH_ENFORCED_IWYU='/src/((bench|common|consensus|crypto|index|kernel|primitives|script|test/fuzz|test/fuzz/util|univalue/(lib|test)|util|zmq)/.*|node/(blockstorage|interfaces|miner|mining_args|utxo_snapshot)|rpc/mining|clientversion|core_io|signet|init)\.cpp'
  ...
  run_iwyu() {
    mv "${BASE_BUILD_DIR}/$1" "${BASE_BUILD_DIR}/compile_commands.json"
    {
      python3 "/include-what-you-use/iwyu_tool.py" \
             ...
             -Xiwyu --check_also='*/test/fuzz/util/wallet\.h' \
             2>&1 || true
    } | tee /tmp/iwyu_ci.out
