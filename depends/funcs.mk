...
$(eval $(1)_all_file_checksums:=$(shell $(build_SHA256SUM) $(meta_depends) packages/$(1).mk $$(sed -n 's/^include[[:space:]]\+\([^[:space:]]*\).*/\1/p' packages/$(1).mk | xargs) $(addprefix $($(1)_patches_path)/,$($(1)_patches)) | cut -d" " -f1))
