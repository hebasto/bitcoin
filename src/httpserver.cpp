...
void HTTPServer::SocketHandlerConnected(const IOReadiness& io_readiness) const
{
    for (const auto& [sock, events] : io_readiness.events_per_sock) {
        ...
        bool send_ready = events.occurred & Sock::BITCOIN_SEND;
        bool recv_ready = events.occurred & Sock::BITCOIN_RECV;
        bool err_ready = events.occurred & Sock::BITCOIN_ERR;
...
HTTPServer::IOReadiness HTTPServer::GenerateWaitSockets() const
{
    ...
    for (const auto& sock : m_listen) {
        io_readiness.events_per_sock.emplace(sock, Sock::Events{Sock::BITCOIN_RECV});
    }
    ...
    for (const auto& http_client : m_connected) {
        ...
        Sock::Event event = (http_client->m_send_ready ? Sock::BITCOIN_SEND : Sock::BITCOIN_RECV);
