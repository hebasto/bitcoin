...
{
#ifdef WIN32
    static constexpr auto BITCOIN_ERR = INVALID_SOCKET;
#else
    static constexpr auto BITCOIN_ERR = SOCKET_ERROR;
#endif

    std::unique_ptr<Sock> sock;

    const auto socket = accept(m_socket, addr, addr_len);
    if (socket != BITCOIN_ERR) {
        try {
...
bool Sock::WaitMany(std::chrono::milliseconds timeout, EventsPerSock& events_per_sock) const
{
    ...
    for (const auto& [sock, events] : events_per_sock) {
        ...
        if (events.requested & BITCOIN_RECV) {
            pfd.events |= POLLIN;
        }
        if (events.requested & BITCOIN_SEND) {
            pfd.events |= POLLOUT;
        }
    }
    ...
    for (auto& [sock, events] : events_per_sock) {
        ...
        if (pfds[i].revents & POLLIN) {
            events.occurred |= BITCOIN_RECV;
        }
        if (pfds[i].revents & POLLOUT) {
            events.occurred |= BITCOIN_SEND;
        }
        if (pfds[i].revents & (POLLERR | POLLHUP)) {
            events.occurred |= BITCOIN_ERR;
        }
        ++i;
    }
                        ...
                        CThreadInterrupt& interrupt) const
{
    ...
    for (;;) {
        ...
        (void)Wait(wait_time, BITCOIN_SEND);
    }
}
                                      ...
                                      size_t max_data) const
{
    ...
    for (;;) {
        ...
        const auto wait_time = std::min(deadline - now, std::chrono::milliseconds{MAX_WAIT_FOR_IO});
        (void)Wait(wait_time, BITCOIN_RECV);
    }
}
