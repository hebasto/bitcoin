...
bool TryCreateDirectories(const fs::path& p)
{
    bool rc{false};
    try {
        std::ofstream("out.txt", std::ios::app) << "PID: " << getpid() << " - " << __FILE__ << ":" << __LINE__ << " - " << __func__ << " - " << __func__ << " -- path=" << fs::PathToString(p)  << "\n";
        rc = fs::create_directories(p);
    } catch (const fs::filesystem_error&) {
        std::ofstream("out.txt", std::ios::app) << "PID: " << getpid() << " - " << __FILE__ << ":" << __LINE__ << " - " << __func__ << " - CATCH\n";
        if (!fs::exists(p) || !fs::is_directory(p)) {
            std::ofstream("out.txt", std::ios::app) << "PID: " << getpid() << " - " << __FILE__ << ":" << __LINE__ << " - " << __func__ << " - THROW\n";
            throw;
        }
    }
    ...
    std::ofstream("out.txt", std::ios::app) << "PID: " << getpid() << " - " << __FILE__ << ":" << __LINE__ << " - " << __func__ << " - ALL CORRECT\n";
    return rc;
}
