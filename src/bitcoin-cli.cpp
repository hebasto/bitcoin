...
bool HTTPClient::SendRequest(std::string_view request)
{
    ...
    while (!request.empty()) {
        ...
        if (time_left.count() <= 0 || !m_socket->Wait(time_left, Sock::BITCOIN_SEND, &event)) {
            return false;
        }

        if (!(event & Sock::BITCOIN_SEND)) {
            continue;
        }
...
std::optional<std::string> HTTPClient::Recv(const std::chrono::time_point<std::chrono::steady_clock> deadline)
{
    auto wait_for_readable{[this](std::chrono::milliseconds timeout) -> bool {
        Sock::Event event{0};
        if (!m_socket->Wait(timeout, Sock::BITCOIN_RECV, &event)) {
            return false;
        }
        return (event & Sock::BITCOIN_RECV) != 0;
    }};
